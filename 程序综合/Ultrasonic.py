...
def main():
    ...
    def servo_color_carstate():
        ...
        if leftdistance < 15 and rightdistance < 15 and frontdistance < 15:
            #亮品红色，掉头
            GPIO.output(LED_R, GPIO.HIGH)
            GPIO.output(LED_G, GPIO.LOW)
    	...
    	if distance > 50:
    	    ...
    	    #遇到障碍物,红外避障模块的指示灯亮,端口电平为LOW
                ...
                if LeftSensorValue == True and RightSensorValue == True :
                    ...
                    spin_left(10, 10)     #右边探测到有障碍物，有信号返回，原地向左转
                    time.sleep(0.002)
                elif RightSensorValue == True and LeftSensorValue == False:
                    spin_right(10, 10)    #左边探测到有障碍物，有信号返回，原地向右转
                    time.sleep(0.002)				
                elif RightSensorValue == False and LeftSensorValue == False :
                    spin_right(10, 10)    #当两侧均检测到障碍物时调用固定方向的避障(原地右转)
                    time.sleep(0.002)
                    run(20, 20)
    	elif distance < 15:
